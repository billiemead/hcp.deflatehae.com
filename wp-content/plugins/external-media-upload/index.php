...
class emu {
	...
	*/
	...
	function save_external_files( $post_id = 0 , $url = null ){
		...
		if( $validLink == true ){
			...
			$ext = substr($path_info['path'], strrpos($path_info['path'], '.')+1);
			$ext = !empty( $ext ) ? $ext : 'jpg';


			$name = pathinfo( $url, PATHINFO_FILENAME )  . '.' . sanitize_text_field( $ext );
			...
			$data['file_size'] = $this->format_size( filesize( get_attached_file( $id ) ) );
	...
	*/
	...
	function checkValidLink( $link ){
		...
		$allow_files = array( 'HTTP/1.1 200 OK' , 'HTTP/1.0 200 OK' , 'HTTP/2 200' );
