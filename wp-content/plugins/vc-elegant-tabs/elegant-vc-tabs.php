...
if ( ! defined( 'ELEGANT_TABS_VC_VERSION' ) ) {
	define( 'ELEGANT_TABS_VC_VERSION', '3.6.7' );
}
...
if ( ! class_exists( 'Elegant_VC_Tabs' ) ) {
	class Elegant_VC_Tabs {
		...
		public function __construct() {
			$this->version_number = ELEGANT_TABS_VC_VERSION;
		...
		public function render_child_tab( $atts, $content = null ) {
			...
			$show_tab = true;
			if ( isset( $atts['hide_tab_users'] ) && 'none' !== $atts['hide_tab_users'] ) {
				if ( 'logged_in' === $atts['hide_tab_users'] ) {
				...
				if ( 'logged_out' === $atts['hide_tab_users'] ) {
					if ( is_user_logged_in() ) {
						$show_tab = true;
					} else {
						$show_tab = false;
		...
		public function render_parent_tab( $atts, $content ) {
			...
			$tab_heading_html        = '';
			$tab_heading_mobile_html = '';
			if ( isset( $atts['tab_heading'] ) && '' !== $atts['tab_heading'] ) {
				$tab_heading_html .= '<h4 class="et-tab-heading">' . $atts['tab_heading'] . '</h4>';
			}

			if ( isset( $atts['tab_sub_heading'] ) && '' !== $atts['tab_sub_heading'] ) {
				$tab_heading_html .= '<span class="et-tab-sub-heading">' . $atts['tab_sub_heading'] . '</span>';
			}

			if ( '' !== $tab_heading_html ) {
				if ( isset( $atts['tab_heading_link'] ) && '' !== $atts['tab_heading_link'] ) {
					$tab_heading_link        = vc_build_link( $atts['tab_heading_link'] );
					$tab_heading_link_target = trim( $tab_heading_link['target'] );
					$tab_heading_link        = $tab_heading_link['url'];
				} else {
					$tab_heading_link = 'javascript:void(0);';
				}

				$tab_heading_color = '';
				if ( isset( $atts['tab_heading_color'] ) && '' !== $atts['tab_heading_color'] ) {
					$tab_heading_color = $atts['tab_heading_color'];
				}

				$tab_heading_mobile_html = '<div class="tab-heading tab-heading-mobile"><a style="color: ' . $tab_heading_color . ';" href="' . $tab_heading_link . '" class="et-tab-heading-wrap">' . $tab_heading_html . '</a></div>';
				$tab_heading_html        = '<li class="tab-heading"><a style="color: ' . $tab_heading_color . ';" href="' . $tab_heading_link . '" class="et-tab-heading-wrap">' . $tab_heading_html . '</a></li>';
			}
		...
		public function integrate_with_vc() {
			...
			$settings = array(
				...
				'base'              => 'et_parent',
				'category'          => esc_attr__( 'InfiWebs', 'infiwebs' ),
				'description'       => esc_attr__( 'Create nice looking tabs .', 'infiwebs' ),
				'icon'              => '',
				'class'             => '',
				'is_container'      => true,
				'as_parent'         => array(
				...
				'params'            => array(
					array(
						'type'        => 'dropdown',
						'heading'     => esc_attr__( 'Tab Style', 'infiwebs' ),
						'param_name'  => 'tab_style',
						'value'       => apply_filters( 'elegant_tab_styles', array() ),
						'description' => esc_attr__( 'Choose the tabs layout you would like to use.', 'infiwebs' ),
					),
					array(
						'type'        => 'checkbox',
						'class'       => '',
						'heading'     => esc_attr__( 'Auto Switch Tabs', 'infiwebs' ),
						'param_name'  => 'auto_switch',
						'value'       => array( 'Check to enable auto switch tabs with interval' => 'yes' ),
						'description' => esc_attr__( 'Check if you want to auto switch tabs with interval.', 'infiwebs' ),
					),
					array(
						'type'        => 'textfield',
						'heading'     => esc_attr__( 'Enter Interval for auto switch tabs', 'infiwebs' ),
						'param_name'  => 'switch_interval',
						'value'       => '5',
						'description' => esc_attr__( 'Enter the interval in seconds you want the tabs to auto switch. eg. 5', 'infiwebs' ),
						'dependency'  => array(
							'element' => 'auto_switch',
							'value'   => array( 'yes' ),
						),
					...
					array(
						'type'        => 'dropdown',
						'class'       => '',
						'heading'     => esc_attr__( 'Tab Click Action', 'infiwebs' ),
						'param_name'  => 'tab_click_action',
						'value'       => array(
							'Display Tab Content' => 'display_content',
							'Open Custom Link'    => 'open_link',
						),
						'description' => esc_attr__( 'Display content - Show tab content on tab click.<br/>Open link - Set custom link in each tab to open the link on tab click. This will also set the tabs to switch on hover.', 'infiwebs' ),
					),
					array(
						'type'        => 'checkbox',
						'class'       => '',
						'heading'     => esc_attr__( 'Show/Hide Tabs on Click', 'infiwebs' ),
						'param_name'  => 'show_hide_tabs',
						'value'       => array( 'Check to Show and Hide active tab on click' => 'yes' ),
						'description' => esc_attr__( 'If you click on active tab, it will be closed on click and opened on clicking again.', 'infiwebs' ),
						'dependency'  => array(
					...
					array(
						'type'        => 'checkbox',
						'class'       => '',
						'heading'     => esc_attr__( 'Show/Hide Accordion on Click', 'infiwebs' ),
						'param_name'  => 'show_hide_accordion',
						'value'       => array( 'Check to Show and Hide active accordion on click' => 'yes' ),
						'description' => esc_attr__( 'If you click on active accordion, it will be closed on click and opened on clicking again.', 'infiwebs' ),
						'dependency'  => array(
							'element' => 'tab_click_action',
							'value'   => array( 'display_content' ),
						),
					...
					array(
						'type'        => 'checkbox',
						'class'       => '',
						'heading'     => esc_attr__( 'Equal Width Tabs', 'infiwebs' ),
						'param_name'  => 'equal_width_tabs',
						'value'       => array( 'Check to set tabs to be equal width' => 'yes' ),
						'description' => esc_attr__( 'Check if you want to make all tabs in this set to be equal width based on the larger tab width.', 'infiwebs' ),
					),
					array(
						'type'        => 'checkbox',
						'class'       => '',
						'heading'     => esc_attr__( 'Switch Tabs on Hover', 'infiwebs' ),
						'param_name'  => 'switch_on_hover',
						'value'       => array( 'Check to set tabs to switch on mouse hover' => 'yes' ),
						'description' => esc_attr__( 'Check if you want to set the tabs to be switched when user hover his mouse on the tab.', 'infiwebs' ),
						'dependency'  => array(
							'element' => 'tab_click_action',
							'value'   => array( 'display_content' ),
						),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_attr__( 'Default Active Tab Number', 'infiwebs' ),
						'param_name'  => 'active_tab',
						'value'       => '1',
						'description' => esc_attr__( 'Enter the number of the tab to be active on load. Enter 0 to hide all tab content on load.', 'infiwebs' ),
					),
					array(
						'type'        => 'dropdown',
						'class'       => '',
						'heading'     => esc_attr__( 'Tab Type', 'infiwebs' ),
						'param_name'  => 'tab_type',
						'value'       => array(
							esc_attr__( 'Hotizontal Tabs', 'infiwebs' ) => 'horizontal',
							esc_attr__( 'Vertical Tabs', 'infiwebs' ) => 'vertical',
						),
						'description' => esc_attr__( 'How would you like to display these tabs?', 'infiwebs' ),
						'dependency'  => array(
							'element' => 'tab_style',
							'value'   => array(
								'bars',
					...
					array(
						'type'        => 'checkbox',
						'class'       => '',
						'heading'     => esc_attr__( 'Justified Tabs', 'infiwebs' ),
						'param_name'  => 'justified',
						'value'       => 'yes',
						'default'     => 'no',
						'description' => esc_attr__( 'This will set all tabs with same justified width. Only works with horizontal tabs.', 'infiwebs' ),
					),
					array(
						'type'        => 'dropdown',
						'class'       => '',
						'heading'     => esc_attr__( 'Tab Alignment', 'infiwebs' ),
						'param_name'  => 'tab_align',
						'value'       => array(
							esc_attr__( 'Left Aligned Tabs', 'infiwebs' ) => 'left',
							esc_attr__( 'Right Aligned Tabs', 'infiwebs' ) => 'right',
							esc_attr__( 'Centered Tabs', 'infiwebs' ) => 'center',
						),
						'description' => esc_attr__( 'Align your tabs. Works only for horizontal tab type.', 'infiwebs' ),
					),
					array(
						'type'        => 'dropdown',
						'class'       => '',
						'heading'     => esc_attr__( 'Tab Content Animation', 'infiwebs' ),
						'param_name'  => 'tab_animation',
						'value'       => array(
							esc_attr__( 'No Animation', 'infiwebs' ) => '',
							esc_attr__( 'Swing', 'infiwebs' )      => 'swing',
							esc_attr__( 'Pulse', 'infiwebs' )      => 'pulse',
							esc_attr__( 'Flash', 'infiwebs' )      => 'flash',
							esc_attr__( 'Fade In', 'infiwebs' )    => 'fadeIn',
							esc_attr__( 'Fade In Up', 'infiwebs' ) => 'fadeInUp',
							esc_attr__( 'Fade In Down', 'infiwebs' ) => 'fadeInDown',
							esc_attr__( 'Fade In Left', 'infiwebs' ) => 'fadeInLeft',
							esc_attr__( 'Fade In Right', 'infiwebs' ) => 'fadeInRight',
							esc_attr__( 'Fade In Up Long', 'infiwebs' ) => 'fadeInUpBig',
							esc_attr__( 'Fade In Down Long', 'infiwebs' ) => 'fadeInDownBig',
							esc_attr__( 'Fade In Left Long', 'infiwebs' ) => 'fadeInLeftBig',
							esc_attr__( 'Fade In Right Long', 'infiwebs' ) => 'fadeInRightBig',
							esc_attr__( 'Slide In Down', 'infiwebs' ) => 'slideInDown',
							esc_attr__( 'Slide In Up', 'infiwebs' ) => 'slideInUp',
							esc_attr__( 'Slide In Left', 'infiwebs' ) => 'slideInLeft',
							esc_attr__( 'Bounce In', 'infiwebs' )  => 'bounceIn',
							esc_attr__( 'Bounce In Up', 'infiwebs' ) => 'bounceInUp',
							esc_attr__( 'Bounce In Down', 'infiwebs' ) => 'bounceInDown',
							esc_attr__( 'Bounce In Left', 'infiwebs' ) => 'bounceInLeft',
							esc_attr__( 'Bounce In Right', 'infiwebs' ) => 'bounceInRight',
							esc_attr__( 'Rotate In', 'infiwebs' )  => 'rotateIn',
							esc_attr__( 'Light Speed In', 'infiwebs' ) => 'lightSpeedIn',
							esc_attr__( 'Roll In', 'infiwebs' )    => 'rollIn',
						),
						'description' => esc_attr__( 'Animate your tab content when it appears!', 'infiwebs' ),
					),
					array(
						'type'        => 'dropdown',
						'class'       => '',
						'heading'     => esc_attr__( 'Tabs on mobile devices', 'infiwebs' ),
						'param_name'  => 'tab_to_mobile',
						'value'       => array(
							esc_attr__( 'Dropdown Select Box', 'infiwebs' ) => 'select',
							esc_attr__( 'Tabs to Accordion', 'infiwebs' )   => 'accordion',
							esc_attr__( 'Inherit Desktop Tabs', 'infiwebs' )   => 'inherit',
						),
						'default'     => 'select',
						'description' => esc_attr__( 'Select how tabs should behave on mobile devices. Inherit option will display the tabs on mobile same as on desktop.', 'infiwebs' ),
					),
					array(
						'type'        => 'checkbox',
						'heading'     => esc_attr__( 'Convert tab navigation to carousel if doesn\'t fit in container', 'infiwebs' ),
						'param_name'  => 'tabs_to_carousel',
						'value'       => 'yes',
						'default'     => 'no',
						'description' => esc_attr__( 'Check to set tabs navigation converted to carousel if it doesn\'t fit in the container. Does not work with vertical tabs.', 'infiwebs' ),
					),
					array(
						'type'        => 'checkbox',
						'heading'     => esc_attr__( 'Add accordion + and - icons indicator', 'infiwebs' ),
						'param_name'  => 'accordion_icons',
						'value'       => 'yes',
						'default'     => 'no',
						'description' => esc_attr__( 'Check to set the + and - icons indicator for the accordions. This is just for UI purpose to let user know there is something to click and expand.', 'infiwebs' ),
						'dependency'  => array(
					...
					array(
						'type'        => 'checkbox',
						'heading'     => esc_attr__( 'Close All Accordions on Load', 'infiwebs' ),
						'param_name'  => 'close_accordions',
						'value'       => 'yes',
						'default'     => 'no',
						'description' => esc_attr__( 'Check to set all accordions closed by default on page load on mobile.', 'infiwebs' ),
						'dependency'  => array(
							'element' => 'tab_to_mobile',
							'value'   => array(
								'accordion',
					...
					array(
						'type'        => 'checkbox',
						'heading'     => esc_attr__( 'Make tabs sticky on scroll', 'infiwebs' ),
						'param_name'  => 'sticky_tabs',
						'value'       => array( esc_attr__( 'Yes', 'infiwebs' ) => 'yes' ),
						'class'       => '',
						'description' => esc_attr__( 'Check to set tabs navigation to be sticky like sticky header on scroll. You need to set the header height in below option to support theme sticky header. Does not work with vertical tabs and on mobile devices due to technical limitations.', 'infiwebs' ),
					),
					array(
						'type'        => 'checkbox',
						'heading'     => esc_attr__( 'Random Active Tab', 'infiwebs' ),
						'param_name'  => 'random_active_tab',
						'value'       => array( esc_attr__( 'Yes', 'infiwebs' ) => 'yes' ),
						'class'       => '',
						'description' => esc_attr__( 'Check to set any random tab as active on page load. This option overrides the custom active tab option.', 'infiwebs' ),
					),
					array(
						'type'        => 'textfield',
						'heading'     => esc_attr__( 'Header height for sticky tabs', 'infiwebs' ),
						'param_name'  => 'header_height',
						'value'       => '100px',
						'description' => esc_attr__( 'Provide the header height of your theme. Inspect the logo header and hover over on the inspected element to get the height. eg. 100px.', 'infiwebs' ),
						'dependency'  => array(
							'element' => 'sticky_tabs',
							'value'   => array( 'yes' ),
						),
					...
					array(
						'type'        => 'textfield',
						'heading'     => esc_attr__( 'Heading Text', 'infiwebs' ),
						'description' => esc_attr__( 'Enter heading text if you want to add a heading for the tabs.', 'infiwebs' ),
						'param_name'  => 'tab_heading',
						'group'       => esc_attr__( 'Tab Heading', 'infiwebs' ),
					),
					array(
						'type'        => 'textfield',
						'heading'     => esc_attr__( 'Sub Heading', 'infiwebs' ),
						'description' => esc_attr__( 'Enter sub-heading text if you want to add a description for the tab heading.', 'infiwebs' ),
						'param_name'  => 'tab_sub_heading',
						'group'       => esc_attr__( 'Tab Heading', 'infiwebs' ),
					),
					array(
						'type'        => 'vc_link',
						'heading'     => esc_attr__( 'Tab Heading Link', 'infiwebs' ),
						'description' => esc_attr__( 'Select link if you want to link the tab heading.', 'infiwebs' ),
						'param_name'  => 'tab_heading_link',
						'group'       => esc_attr__( 'Tab Heading', 'infiwebs' ),
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Tab Heading Text Color', 'infiwebs' ),
						'param_name'  => 'tab_heading_color',
						'value'       => '',
						'description' => esc_attr__( 'The font color of the tab heading text and sub-heading text.', 'infiwebs' ),
						'group'       => esc_attr__( 'Tab Heading', 'infiwebs' ),
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Tab Text Color', 'infiwebs' ),
						'param_name'  => 'color_tab_txt',
						'value'       => '',
						'description' => esc_attr__( 'The font color of the inactive Tab in this set.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Tab Background/Border Color', 'infiwebs' ),
						'param_name'  => 'color_tab_bg',
						'value'       => '',
						'description' => esc_attr__( 'The background color of the inactive Tab in this set..', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Active Tab Text Color', 'infiwebs' ),
						'param_name'  => 'color_act_txt',
						'value'       => '',
						'description' => esc_attr__( 'The font color of the active Tab in this set.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Active Tab Background Color', 'infiwebs' ),
						'param_name'  => 'color_act_bg',
						'value'       => '',
						'description' => esc_attr__( 'The background color of the active Tab in this set.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Tab Hover Text Color', 'infiwebs' ),
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Tab Hover Background Color', 'infiwebs' ),
					...
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Tab Content Background Color', 'infiwebs' ),
						'param_name'  => 'color_content_bg',
						'value'       => '#f4f4f4',
						'description' => esc_attr__( 'The background color of the Tab Content Area.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Tab Content Text Color', 'infiwebs' ),
						'param_name'  => 'color_content_txt',
						'value'       => '#444444',
						'description' => esc_attr__( 'The text color of the Tab Content Area.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'textfield',
						'heading'     => esc_attr__( 'Tab Title Font Size', 'infiwebs' ),
						'param_name'  => 'title_font_size',
						'value'       => '',
						'description' => esc_attr__( 'Provide the font size for the tab title. eg. 16px.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Arrow Color', 'infiwebs' ),
						'param_name'  => 'color_arrow',
						'value'       => '',
						'description' => esc_attr__( 'The font color of the arrow icon when tabs switched to carousel.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'colorpicker',
						'heading'     => esc_attr__( 'Arrow Background Color', 'infiwebs' ),
						'param_name'  => 'color_arrow_bg',
						'value'       => '',
						'description' => esc_attr__( 'The background color of the arrow icon when tabs switched to carousel.', 'infiwebs' ),
						'group'       => 'Design',
					),
					array(
						'type'        => 'textfield',
						'heading'     => esc_attr__( 'CSS class name', 'infiwebs' ),
						'param_name'  => 'el_class',
						'description' => esc_attr__( 'Give this element an extra CSS class name if you wish to refer to it in a CSS file. (optional)', 'infiwebs' ),
					),
				),
				...
				'default_content'   => '[et_single tab_title="' . esc_attr__( 'Tab 1', 'infiwebs' ) . '" tab_id="' . $tab_id_1 . ' ][/et_single]
				[et_single tab_title="' . esc_attr__( 'Tab 2', 'infiwebs' ) . '" tab_id="' . $tab_id_2 . ' ][/et_single]
				',
			);
			...
			if ( function_exists( 'vc_map' ) ) {
				vc_map(
					array(
						'name'                      => esc_attr__( 'Single Tab', 'infiwebs' ),
						'base'                      => 'et_single',
						'icon'                      => '',
						'class'                     => '',
						...
						'params'                    => array(
							array(
								'type'       => 'textfield',
								'heading'    => esc_attr__( 'Title', 'infiwebs' ),
								'param_name' => 'tab_title',
							),
							array(
								'type'       => 'textfield',
								'heading'    => esc_attr__( 'Sub Title', 'infiwebs' ),
								'param_name' => 'tab_sub_title',
							),
							array(
								'type'        => 'vc_link',
								'heading'     => esc_attr__( 'Tab Link', 'infiwebs' ),
								'description' => esc_attr__( 'Select link only if you have set the tab action to open custom link in parent tabs settings.', 'infiwebs' ),
								'param_name'  => 'tab_link',
							),
							array(
								'type'        => 'checkbox',
								'class'       => '',
								'heading'     => esc_attr__( 'Double tap to open link', 'infiwebs' ),
								'param_name'  => 'double_tap_click',
								'value'       => array( 'Check to open link on double tap for touch screen devices' => 'yes' ),
								'description' => esc_attr__( 'This will enable the tab with custom link to show content on first tap on touch screen devices.', 'infiwebs' ),
							),
							array(
								'heading'     => esc_attr__( 'Icon Type', 'infiwebs' ),
								'description' => esc_attr__( 'Would you like to use font icons or custom image icon?', 'infiwebs' ),
								'value'       => array(
									esc_attr__( 'Font Icon', 'infiwebs' )  => 'icon',
									esc_attr__( 'Image Icon', 'infiwebs' ) => 'img_icon',
								),
								'type'        => 'dropdown',
								'param_name'  => 'icon_type',
							),
							array(
								'heading'     => esc_attr__( 'Icon', 'infiwebs' ),
								'description' => esc_attr__( 'Select the icon you would like to use for this tab.', 'infiwebs' ),
								'value'       => 'hand-o-right',
								'type'        => $icon_type,
								'settings'    => array(
							...
							array(
								'heading'     => esc_attr__( 'Custom Image Icon', 'infiwebs' ),
								'description' => esc_attr__( 'Upload the custom image icon you would like to use for this tab.', 'infiwebs' ),
								'value'       => '',
								'type'        => 'attach_image',
								'param_name'  => 'icon_img',
								'dependency'  => array(
							...
							array(
								'heading'     => esc_attr__( 'Custom Image Icon on Hover', 'infiwebs' ),
								'description' => esc_attr__( 'Upload the custom image icon you would like to use for this tab to display on hover state.', 'infiwebs' ),
								'value'       => '',
								'type'        => 'attach_image',
							...
							array(
								'heading'     => esc_attr__( 'Image Icon Width', 'infiwebs' ),
								'description' => esc_attr__( 'Set the custom image icon width. Default is - 32px.', 'infiwebs' ),
								'value'       => '',
								'type'        => 'textfield',
							...
							array(
								'heading'     => esc_attr__( 'Image Icon Height', 'infiwebs' ),
								'description' => esc_attr__( 'Set the custom image icon height. Default is - 32px.', 'infiwebs' ),
								'value'       => '',
								'type'        => 'textfield',
							...
							array(
								...
								'heading'          => esc_attr__( 'Tab ID', 'infiwebs' ),
								'param_name'       => 'tab_id',
							),
							array(
								'type'             => 'info',
								...
								'edit_field_class' => ' vc_col-sm-12 vc_column wpb_el_type_textfield vc_shortcode-param',
								'heading'          => '<span style="color:#f3af1c;background: #f6f6f6;display: block;padding: 15px;border-bottom: 2px solid;font-weight: 400;"><strong>Please Note:</strong> <br/><br/>Changing tab colors from here will override the colors from parent. Also, parent global and hover colors might not work for all tab styles.</span>',
								'group'            => 'Design',
							),
							array(
								'type'        => 'colorpicker',
							...
							array(
								'type'        => 'colorpicker',
								'heading'     => esc_attr__( 'Tab Background Color', 'infiwebs' ),
								'param_name'  => 'color_tab_bg',
								'value'       => '',
								'description' => esc_attr__( 'The background color of the inactive Tab in this set..', 'infiwebs' ),
								'group'       => 'Design',
							...
							array(
								'heading'     => esc_attr__( 'Hide Tab From', 'infiwebs' ),
								'description' => esc_attr__( 'Would you like hide this tab from certain users?', 'infiwebs' ),
								'value'       => array(
									esc_attr__( 'No One', 'infiwebs' )  => 'none',
									esc_attr__( 'Logged In Users', 'infiwebs' ) => 'logged_in',
									esc_attr__( 'Logged Out Users', 'infiwebs' ) => 'logged_out',
								),
								'type'        => 'dropdown',
								'param_name'  => 'hide_tab_users',
								'group'       => esc_attr__( 'Conditional Tab', 'infiwebs' ),
							),
						),
					)
				);
		...
		public function et_tabs_admin() {
			...
			if ( defined( 'US_CORE_VERSION' ) ) {
				wp_register_style( 'font-awesome', plugins_url( 'css/font-awesome.min.css', __FILE__ ), '', $this->version_number );
...
if ( class_exists( 'WPBakeryShortCode' ) ) {
	...
	define( 'ET_TAB_TITLE', esc_attr__( 'Tab', 'infiwebs' ) );
