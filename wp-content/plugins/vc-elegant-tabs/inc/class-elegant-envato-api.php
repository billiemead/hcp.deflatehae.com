 ...
 * Envato API class.
 *
 * @package Elegant_Tabs_VC_Envato_API
 */
 ...
 *
 * @class Elegant_Tabs_VC_Envato_API
 * @version 1.0.0
 * @since 3.5.0
 */
class Elegant_Tabs_VC_Envato_API {
	...
	public function __clone() {
		_doing_it_wrong( __FUNCTION__, esc_html__( 'Cheatin&#8217; huh?', 'elegant-tabs' ), '1.0.0' );
	}
	...
	public function request( $url, $args = array() ) {
		...
		if ( empty( $this->token ) ) {
			return new WP_Error( 'api_token_error', esc_attr__( 'An API token is required.', 'elegant-tabs' ) );
		}
		...
		if ( 200 !== $response_code ) {
			return new WP_Error( $response_code, esc_attr__( 'An unknown API error occurred.', 'elegant-tabs' ) );
		}
		$return = json_decode( wp_remote_retrieve_body( $response ), true );
		if ( null === $return ) {
			return new WP_Error( 'api_error', esc_attr__( 'An unknown API error occurred.', 'elegant-tabs' ) );
		}
		return $return;
	}
