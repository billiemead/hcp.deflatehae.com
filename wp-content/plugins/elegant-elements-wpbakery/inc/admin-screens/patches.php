<div class="wrap about-wrap elegant-elements-wrap">
	...
	<div class="elegant-elements-important-notice">
		<p class="about-description">
			<?php esc_attr_e( 'Elegant Elements for WPBakery Page Builder introduces the patcher tool, which allows our development team to deploy fixes and improvements without the need for our team to prepare and release a full plugin update. All you need to do is, just click the Apply Patch button for each patch available.', 'elegant-elements' ); ?>
			<br/>
		</p>
	...
	<?php
	$current_version = get_option( 'elegant_elements_wpbakery_version' );
	if ( EEWPB_VERSION !== $current_version ) {
	...
	if ( infi_eewpb()->registration->is_registered() ) {
		echo infi_eewpb()->patcher->get_patches();
	} else {
		?>
		<p class="error notice update-nag" style="display: block !important;margin-right: 0;margin-bottom: 30px;">
			<?php echo sprintf( __( 'Looks like you have not registered the plugin yet. Please go to <a href="%s">Registration</a> tab and register your product with just one click.', 'elegant-elements' ), admin_url( '/admin.php?page=elegant-elements-register' ) ); ?>
		</p>
		<?php
	}
	?>
