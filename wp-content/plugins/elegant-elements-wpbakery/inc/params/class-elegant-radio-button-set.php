<?php
/**
 * Radio Button Set param class.
 *
 * @package elegant-elements
 * @since 1.0
 */
class ElegantRadioButtonSet {
	...
	public function __construct() {
		add_action(
			...
			array(
				$this,
				'vc_load_elegant_ee_radio_button_set',
			)
		);
	...
	public function vc_enqueue_editor_scripts_backend() {
		wp_enqueue_script( 'eewpb-radio-buttonset-backend', preg_replace( '/\s/', '%20', plugins_url( 'inc/params/js/radio-button-set.js', EEWPB_PLUGIN_FILE ) ), array( 'jquery' ), EEWPB_VERSION, true );
	}
	...
	public function vc_enqueue_editor_scripts_frontend() {
		wp_enqueue_script( 'eewpb-radio-buttonset-frontend', preg_replace( '/\s/', '%20', plugins_url( 'inc/params/js/radio-button-set.js', EEWPB_PLUGIN_FILE ) ), array( 'jquery' ), EEWPB_VERSION, true );
	}

	/**
	 * Add custom param to system
	 */
	public function vc_load_elegant_ee_radio_button_set() {
	...
	public function render( $settings, $value ) {
		...
		$values = isset( $settings['value'] ) && is_array( $settings['value'] ) ? $settings['value'] : array(
			...
			esc_attr__( 'No', 'elegant-elements' )  => 'false',
		);
		...
		if ( ! empty( $values ) ) {
			...
			<div class="elegant-option-field">
				<div class="eewpb-option-radio-button-set ui-buttonset eewpb-option-<?php echo esc_attr( $param_name ); ?>">
					<?php
					$choice = $value;
					$index  = 0;
					...
					foreach ( $values as $title => $value ) {
						$index++;
						$button_class  = ( $value === $choice ) ? ' ui-state-active' : '';
						...
						if ( '' !== $icon && false !== strpos( $icon, 'svg' ) ) {
							$title = $icon;
						} if ( '' !== $icon && false !== strpos( $icon, 'jpg' ) ) {
							$title         = '<img src="' . $icon . '">';
							$button_class .= ' radio-image-icon';
						}
						?>
