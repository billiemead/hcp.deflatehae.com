...
final class ReporterServer {
	...
	public function get_title(): string {
		...
		return esc_html__( 'Server Environment', 'lottier-wpbakery' );
	...
	public function get_fields(): array {
        ...
        $checks = [];
        $checks = $this->add_check( $checks, $server_checks, 'os', esc_html__( 'Operating System', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'software', esc_html__( 'Software', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'mysql_version', esc_attr( 'MySQL' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'php_version', esc_attr( 'PHP' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'write_permissions', esc_html__( 'Write Permissions', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'zip_installed', esc_attr( 'ZIP' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'curl_installed', esc_attr( 'cURL' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'elementor_installed', esc_attr( 'Elementor' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'wpbakery_installed', esc_attr( 'WPBakery' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'allow_url_fopen', esc_attr( 'allow_url_fopen' ) );
        $checks = $this->add_check( $checks, $server_checks, 'dom_installed', esc_attr( 'DOM' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'xml_installed', esc_attr( 'XML' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'bcmath_installed', esc_attr( 'BCMath' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'mbstring_installed', esc_attr( 'mbstring' ) . ' ' . esc_html__( 'Installed', 'lottier-wpbakery' ) );
        $checks = $this->add_check( $checks, $server_checks, 'server_time', esc_html__( 'Server Time Sync' ) );

		return apply_filters( 'mdp_lottier_wpbakery_status_reports', $checks );
	...
	public function get_allow_url_fopen(): array {
		...
		return [
			'value' => $allow_url_fopen ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'lottier-wpbakery' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'lottier-wpbakery' ),
			'warning' => ! $allow_url_fopen,
			'recommendation' => esc_html__( 'You must enable allow_url_fopen option in PHP. Contact the support service of your hosting provider. They know what to do.', 'lottier-wpbakery' )
		];
	...
	public function get_php_version(): array {
		...
		$version = apply_filters( 'lottier_wpbakery_required_php_version', '7.1.0' );
		...
		if ( version_compare( $result['value'], $version, '<' ) ) {
			...
			$result['recommendation'] = wp_sprintf(
				...
				esc_html__( 'We recommend to use PHP %1$s or higher. Your version is %2$s', 'lottier-wpbakery' ),
				$version,
				$result['value']
			);
	...
	public function get_zip_installed(): array {
		...
		return [
			'value' => $zip_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'lottier-wpbakery') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'lottier-wpbakery' ),
	...
	public function get_curl_installed(): array {
		...
		return [
			'value' => $curl_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'lottier-wpbakery' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'lottier-wpbakery' ),
			'warning' => ! $curl_installed,
			'recommendation' => esc_html__( 'You must enable CURL (Client URL Library) in PHP. Contact the support service of your hosting provider. They know what to do.', 'lottier-wpbakery' )
		];
	...
	public function get_elementor_installed(): array {
		...
		return [
			'value' => $elementor_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'lottier-wpbakery') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'lottier-wpbakery' ),
			'warning' => ! $elementor_installed,
			'recommendation' => esc_html__( 'You need install and activate Elementor builder. Go to Elementor site (elementor.com) for details.', 'lottier-wpbakery' )
		];
    ...
    public function get_wpbakery_installed(): array {
        ...
        return [
            'value' => $wpbakery_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'lottier-wpbakery') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-wpbakery' ),
            'warning' => ! $wpbakery_installed,
            'recommendation' => esc_html__('You need install and activate WPBakery builder. Go to WPBakery site (wpbakery.com) for details.', 'lottier-wpbakery' )
        ];
	...
	public function get_dom_installed(): array {
		...
		return [
			'value' => $dom_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'lottier-wpbakery') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'lottier-wpbakery' ),
			'warning' => ! $dom_installed,
			'recommendation' => esc_html__(' You must enable DOM extension (Document Object Model) in PHP. It\'s used for HTML processing. Contact the support service of your hosting provider. They know what to do.', 'lottier-wpbakery' )
		];
	...
	public function get_xml_installed(): array {
		...
		return [
			'value' => $xml_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'lottier-wpbakery' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'lottier-wpbakery' ),
			'warning' => ! $xml_installed,
			'recommendation' => esc_html__( 'You must enable XML extension in PHP. It\'s used for XML processing. Contact the support service of your hosting provider. They know what to do.', 'lottier-wpbakery' )
		];
	...
	public function get_bcmath_installed(): array {
		...
		return [
			'value' => $bcmath_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'YES', 'lottier-wpbakery' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'NO', 'lottier-wpbakery' ),
			'warning' => ! $bcmath_installed,
			'recommendation' => esc_html__( 'You must enable BCMath extension (Arbitrary Precision Mathematics) in PHP. Contact the support service of your hosting provider. They know what to do.', 'lottier-wpbakery' )
		];
    ...
    public function get_mbstring_installed(): array {
        ...
        return [
            'value' => $mbstring_installed ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'lottier-wpbakery' ) : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-wpbakery' ),
            'warning' => ! $mbstring_installed,
            'recommendation' => esc_html__('You must enable mbstring extension (Multibyte String) in PHP. Contact the support service of your hosting provider. They know what to do.', 'lottier-wpbakery' )
        ];
    }
    ...
    public function get_server_time(): array {
        ...
        if ( $curl_errno ) {
            return [
                'value' => '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-wpbakery' ),
                'warning' => true,
                'recommendation' => esc_html__('Failed to check time synchronization on your server. Your server\'s clock must be in sync with network time protocol - NTP.', 'lottier-wpbakery' )
            ];
        }
        ...
        return [
            'value' => $time_ok ? '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__('YES', 'lottier-wpbakery') : '<i class="material-icons mdc-system-no">error</i>' . esc_html__('NO', 'lottier-wpbakery' ) . '<br>Google Time: ' . $google_time . '<br>&nbsp;&nbsp;&nbsp;Local Time: ' . $server_time,
            'warning' => ! $time_ok,
            'recommendation' => esc_html__( ' Your server\'s clock is not in sync with network time protocol - NTP. Contact the support service of your hosting provider. They know what to do.', 'lottier-wpbakery' )
        ];
	...
	public function get_write_permissions(): array {
		...
		$paths_to_check = [
			ABSPATH => esc_html__( 'WordPress root directory', 'lottier-wpbakery' )
		];
		...
		if ( $wp_upload_dir[ 'error' ] ) {
			$write_problems[] = esc_html__( 'WordPress root uploads directory', 'lottier-wpbakery' );
		}

		$htaccess_file = ABSPATH . '/.htaccess';

		if ( file_exists( $htaccess_file ) ) {
			$paths_to_check[ $htaccess_file ] = esc_html__( '.htaccess file', 'lottier-wpbakery' );
		}
		...
		if ( $write_problems ) {
			...
			$value = '<i class="material-icons mdc-system-no">error</i>' . esc_html__( 'There are some writing permissions issues with the following directories/files:', 'lottier-wpbakery' ) . "<br> &nbsp;&nbsp;&nbsp;&nbsp;– ";
			$value .= implode( "<br> &nbsp;&nbsp;&nbsp;&nbsp;– ", $write_problems );

		} else {

			$value = '<i class="material-icons mdc-system-yes">check_circle</i>' . esc_html__( 'All right', 'lottier-wpbakery' );
