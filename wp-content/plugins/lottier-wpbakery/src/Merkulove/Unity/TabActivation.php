...
final class TabActivation extends Tab {
    ...
    public function add_settings() {
        ...
        /** Activation Tab. */
        register_setting( 'LottierWpbakeryActivationOptionsGroup', 'envato_purchase_code_' . EnvatoItem::get_instance()->get_id() );
        add_settings_section( 'mdp_lottier_wpbakery_settings_page_activation_section', '', null, 'LottierWpbakeryActivationOptionsGroup' );
    ...
    public function do_settings() {
        ...
        /** Render fields. */
        settings_fields( 'LottierWpbakeryActivationOptionsGroup' );
        do_settings_sections( 'LottierWpbakeryActivationOptionsGroup' );
	...
	public function display_status() {
		...
		$activation_tab = admin_url( 'admin.php?page=mdp_lottier_wpbakery_settings&tab=activation' );
		?>

        <hr class="mdc-list-divider">
        <h6 class="mdc-list-group__subheader"><?php esc_html_e( 'CodeCanyon License', 'lottier-wpbakery' ); ?></h6>
		...
		<?php if ( $this->is_activated() ) : ?>
            <a class="mdc-list-item mdp-activation-status activated" href="<?php echo esc_url( $activation_tab ); ?>">
                ...
                <span class="mdc-list-item__text"><?php esc_html_e( 'Activated', 'lottier-wpbakery' ); ?></span>
            </a>
		<?php else : ?>
            <a class=" mdc-list-item mdp-activation-status not-activated" href="<?php echo esc_url( $activation_tab ); ?>">
                ...
                <span class="mdc-list-item__text"><?php esc_html_e( 'Not Activated', 'lottier-wpbakery' ); ?></span>
            </a>
		<?php endif;
	...
	public function remote_validation( $purchase_code ) {
		...
		if ( is_wp_error( $json ) ) {
            ...
            delete_transient( 'mdp_lottier_wpbakery_activation_error' );
            set_transient(
                'mdp_lottier_wpbakery_activation_error',
                $json->errors ?? array(),
                60
            );
	...
	public function render_subscribe() {
        ...
        <div class="mdp-subscribe-form">
            ...
            <h3><?php esc_html_e( 'Subscribe to newsletter', 'lottier-wpbakery' ); ?></h3>
            <p><?php esc_html_e( 'Sign up for the newsletter to be the first to know about news and discounts.', 'lottier-wpbakery' ); ?></p>
            <p class="mdp-subscribe-form-message"
               data-success="<?php esc_html_e( 'Hurray! We received your Subscription request. Check your inbox for an email from us.', 'lottier-wpbakery' ); ?>"
               data-warn="<?php esc_html_e( 'Oh! Sorry, but we cannot send messages to this email.', 'lottier-wpbakery' ); ?>"
               data-error="<?php esc_html_e( 'Oops! Something went wrong. Please try later.', 'lottier-wpbakery' ); ?>"
            ></p>
            ...
            UI::get_instance()->render_input(
                '',
                esc_html__( 'Your Name', 'lottier-wpbakery' ),
                '',
                [
            ...
            UI::get_instance()->render_input(
                '',
                esc_html__( 'Your E-Mail', 'lottier-wpbakery' ),
                '',
                [
            ...
            UI::get_instance()->render_button(
                esc_html__( 'Subscribe', 'lottier-wpbakery' ),
                '',
                false,
                [
                    "name"  => "mdp-subscribe",
	...
	public function render_form() {
        ...
        <div class="mdp-activation-form">
            ...
            UI::get_instance()->render_input(
                $purchase_code,
                esc_html__( 'Purchase code', 'lottier-wpbakery'),
                esc_html__( 'Enter your CodeCanyon purchase code. Allowed only one Purchase Code per website.', 'lottier-wpbakery' ),
                [
    ...
    public function render_FAQ() {
        ...
        <div class="mdp-activation-faq">
            <div class="mdc-accordion" data-mdp-accordion="showfirst: true">
                ...
                <h3><?php esc_html_e( 'Activation FAQ\'S', 'lottier-wpbakery' ); ?></h3>

                <div class="mdc-accordion-title">
                    <i class="material-icons">help</i>
                    <span class="mdc-list-item__text"><?php esc_html_e( 'Where is my Purchase Code?', 'lottier-wpbakery' ); ?></span>
                </div>
                <div class="mdc-accordion-content">
                    <p><?php esc_html_e( 'The purchase code is a unique combination of characters that confirms that you bought the plugin. You can find your purchase code in ', 'lottier-wpbakery' ); ?>
                        <a href="https://1.envato.market/cc-downloads" target="_blank"><?php esc_html_e( 'your account', 'lottier-wpbakery' );?></a><?php esc_html_e( 'on the CodeCanyon. Learn more about ', 'lottier-wpbakery' ); ?>
                        <a href="https://help.market.envato.com/hc/en-us/articles/202822600-Where-Is-My-Purchase-Code-" target="_blank"><?php esc_html_e( 'How to find your purchase code', 'lottier-wpbakery' );?></a>.
                    </p>
                </div>

                <div class="mdc-accordion-title">
                    <i class="material-icons">help</i>
                    <span class="mdc-list-item__text"><?php esc_html_e( 'Can I use one Purchase Code on multiple sites?', 'lottier-wpbakery' ); ?></span>
                </div>
                <div class="mdc-accordion-content">
                    <p>
                        <?php esc_html_e( 'No, this is prohibited by license terms. You can use the purchase code on only one website at a time. Learn more about ', 'lottier-wpbakery' ); ?>
                        <a href="https://1.envato.market/KYbje" target="_blank"><?php esc_html_e( 'Envato License', 'lottier-wpbakery' );?></a> <?php esc_html_e( 'terms. ', 'lottier-wpbakery' ); ?>
                    </p>
                </div>

                <div class="mdc-accordion-title">
                    <i class="material-icons">help</i>
                    <span class="mdc-list-item__text"><?php esc_html_e( 'What are the benefits of plugin activation?', 'lottier-wpbakery' ); ?></span>
                </div>
                <div class="mdc-accordion-content">
                    <p>
                        <?php esc_html_e( 'Activation of the plugin allows you to use all the functionality of the plugin on your site. In addition, in some cases, activating the plugin allows you to access additional features and capabilities of the plugin. Also, using an authored version of the plugin, you can be sure that you will not violate the license.', 'lottier-wpbakery' ); ?>
                    </p>
                </div>

                <div class="mdc-accordion-title">
                    <i class="material-icons">help</i>
                    <span class="mdc-list-item__text"><?php esc_html_e( 'What should I do if my Purchase Code does not work?', 'lottier-wpbakery' ); ?></span>
                </div>
                <div class="mdc-accordion-content">
                    <p>
                        <?php esc_html_e( 'There are several reasons why the purchase code may not work on your site. Learn more why your ', 'lottier-wpbakery' ); ?>
                        <a href="https://merkulove.zendesk.com/hc/en-us/articles/360006100998-Troubleshooting-of-the-plugin-activation" target="_blank"><?php esc_html_e( 'Purchase Code is Not Working', 'lottier-wpbakery' );?></a>.
                    </p>
                </div>
