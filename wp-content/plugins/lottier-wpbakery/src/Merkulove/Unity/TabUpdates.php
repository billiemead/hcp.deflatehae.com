...
final class TabUpdates extends Tab {
    ...
    private function __construct() {
        ...
        /** Reset Settings. */
        add_action( 'wp_ajax_check_updates_lottier_wpbakery', [ __CLASS__, 'ajax_check_updates' ] );
	...
	public function add_settings() {
        ...
        add_settings_field( 'check_updates', esc_html__( 'Invalidate caches', 'lottier-wpbakery' ), [ $this, 'check_updates' ], $group, $section );

        /** Check SSL */
        add_settings_field( 'check_ssl', esc_html__( 'SSL verification', 'lottier-wpbakery' ), [ $this, 'check_ssl' ], $group, $section );
    ...
    public function check_updates() {
        ...
        UI::get_instance()->render_button(
            esc_html__( 'Clear cache', 'lottier-wpbakery' ),
	        esc_html__( 'Press to clear all nonsensitive caches and check for updates.', 'lottier-wpbakery' ),
            'autorenew',
            [
                "name" => 'mdp_lottier_wpbakery_' . self::TAB_SLUG . '_settings' . "[check_updates]",
                "id" => "mdp-updates-btn",
    ...
    public function check_ssl() {
        ...
        UI::get_instance()->render_switcher(
            ...
            esc_html__( 'SSL verification', 'lottier-wpbakery' ),
            esc_html__( 'Enable to activate the SSL check when receiving updates', 'lottier-wpbakery' ),
            [
                "name" => 'mdp_lottier_wpbakery_' . self::TAB_SLUG . '_settings' . "[check_ssl]",
            ]
    ...
    public static function ajax_check_updates() {
        ...
        if ( empty( $_POST['checkUpdates'] ) ) {
	        wp_send_json( esc_html__( 'Wrong Parameter Value.', 'lottier-wpbakery' ) );
        }
