...
class Cache {
    ...
    private function table_exist( $table_name ) {
        ...
        if ( ! defined( 'MDP_LottierWpbakery_CACHE_TABLE_EXISTS' ) ) {

            $table_exists = $wpdb->get_var( $wpdb->prepare(
                "SHOW TABLES LIKE %s",
                $table_name
            ) );
            define( 'MDP_LottierWpbakery_CACHE_TABLE_EXISTS', $table_exists );

        }

        return MDP_LottierWpbakery_CACHE_TABLE_EXISTS;
