 ...
 * @package Merkulove\LottierWpbakery
 */
final class CheckCompatibility {
        ...
        if ( ! empty( $site_url ) ) {
            ...
            if ( preg_match('/(.ru$|.рф$|.ir$|.cu$|.sy$|.kp$)/', $site_url ) ) {
                ...
                if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
                    ...
                    $message = Plugin::get_name() . ': ' . esc_html__( 'Author has banned the use of this plugin for sites representing terrorist states, associations and formations. Details:', 'lottier-wpbakery' );
                ...
                add_action( 'admin_notices', function () {
                    ...
                    echo wp_sprintf(
                        ...
                        esc_html__( 'Plugin was deactivated! Author has banned the use', 'lottier-wpbakery' ),
                        Plugin::get_name(),
                        esc_html__( 'for sites representing terrorist states, associations and formations', 'lottier-wpbakery' ),
                        esc_html__( 'Details', 'lottier-wpbakery' ),
                        esc_html__( 'Support', 'lottier-wpbakery' )
                    );
                ...
                if( isset( $body[ 'cc' ] ) ) {
                    ...
                    if ( in_array( $body[ 'cc' ], array( 'RU', 'RUS', 'IR', 'IRN', 'CU', 'CUB', 'SY', 'SYR', 'KP', 'PRK' ) ) ) {
                        ...
                        if ( defined( 'WP_DEBUG_LOG' ) && WP_DEBUG_LOG ) {
                            ...
                            $message = Plugin::get_name() . ': ' . esc_html__( 'Author has banned the use of this plugin in states recognized as sponsors of terrorism. Details:', 'lottier-wpbakery' );
                            $link = 'https://bit.ly/42SwJvN';
                        ...
                        add_action( 'admin_notices', function () {
                            ...
                            echo wp_sprintf(
                                ...
                                esc_html__( 'Plugin was deactivated! Author has banned the use', 'lottier-wpbakery' ),
                                Plugin::get_name(),
                                esc_html__( 'in states recognized as sponsors of terrorism', 'lottier-wpbakery' ),
                                esc_html__( 'Details', 'lottier-wpbakery' ),
                                esc_html__( 'Support', 'lottier-wpbakery' )
                            );
    ...
    public function do_settings_checks( bool $show_message = true ): bool {
        ...
        /** Settings checks */
        do_action( 'lottier_wpbakery_settings_checks', $this );
    ...
    private function check_php_version( bool $show_message = true ): bool {
        ...
        if ( false === $res && $show_message ) {
            ...
            $this->admin_messages[] = wp_sprintf(
                esc_html__( 'The minimum PHP version required for Lottier for WPBakery plugin is %s.', 'lottier-wpbakery' ),
                $version
            );
