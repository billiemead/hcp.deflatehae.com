...
final class UI {
     ...
     * @return void
     */
	public function render_slider( $value, int $value_min = 0, int $value_max = 10, int $step = 1, string $label = '', string $helper_text = '', array $attributes = [], bool $discrete = true ) {
    ...
    public function render_icon( $value, string $label = '', string $helper_text = '', array $attributes = [], array $meta = [] ) {
        ...
        <div <?php echo ( $class ) ? 'class="' . esc_attr( $class ) . '"' : ' '; ?>>
            ...
            <div class="mdc-icon-field-img-box <?php if ( $src ) : ?>mdc-with-image<?php endif; ?>">
                ...
                ><?php esc_html_e( 'Icon Library', 'lottier-wpbakery' ); ?></button>
                <button class="mdc-icon-field-upload-btn mdc-button mdc-button--outlined mdc-ripple-upgraded"><?php esc_html_e( 'Upload SVG', 'lottier-wpbakery' ); ?></button>
            </div>
	...
	public function render_media_library( $value, string $helper_text = '', array $attributes = [] ) {
		...
		$image = intval( $value ) > 0 ?
			...
			wp_sprintf(
				...
				$id . '-preview-image',
				esc_html__( 'Preview image' , 'lottier-wpbakery' )
			);
		...
		echo wp_sprintf(
			...
			$image,
			esc_attr__( 'Remove', 'lottier-wpbakery' ),
			esc_attr( $id ),
			esc_html__( 'Media library', 'lottier-wpbakery' )
		);
        ...
        <div class="mdp-dnd">
            ...
            <div class="mdc-text-field mdc-input-width mdc-text-field--outlined mdc-hidden">
                ...
                <input  type="text"
                        ...
                        name="mdp_lottier_wpbakery_<?php echo esc_attr( $tab ); ?>_settings[<?php echo esc_attr( $key ); ?>]"
                        id="mdp-lottier-wpbakery-settings-dnd-<?php echo esc_attr( $key ); ?>"
                        value="<?php esc_attr_e( $value ); ?>"
                >
                <div class="mdc-notched-outline mdc-notched-outline--upgraded mdc-notched-outline--notched">
                    <div class="mdc-notched-outline__leading"></div>
                    <div class="mdc-notched-outline__notch">
                        <label for="mdp-lottier-wpbakery-settings-dnd-<?php echo esc_attr( $key ); ?>" class="mdc-floating-label mdc-floating-label--float-above"><?php echo esc_html( $label ); ?></label>
                    </div>
                    <div class="mdc-notched-outline__trailing"></div>
                </div>
            </div>
            <div id="mdp-<?php echo esc_attr( $key ); ?>-drop-zone" class="<?php if ( $key_exist ) : ?>mdp-key-uploaded <?php endif; ?>mdp-drop-zone">
                <?php if ( $key_exist ) : ?>
                    <span class="material-icons">check_circle_outline</span><?php esc_html_e( 'File exist', 'lottier-wpbakery' ); ?>
                    <span class="mdp-drop-zone-hover"><?php esc_html_e( 'Drop file here or click to upload', 'lottier-wpbakery' ); ?></span>
                <?php else : ?>
                    <span class="material-icons">cloud</span><?php esc_html_e( 'Drop file here or click to upload.', 'lottier-wpbakery' ); ?>
                <?php endif; ?>
            </div>
            <?php if ( $key_exist ) : ?>
                <div class="mdp-messages mdc-text-field-helper-line mdc-text-field-helper-text mdc-text-field-helper-text--persistent">
                    <?php esc_html_e( 'Drag and drop or click on the form to replace file. |', 'lottier-wpbakery' ); ?>
                    <a href="#" class="mdp-reset-key-btn"><?php esc_html_e( 'Reset', 'lottier-wpbakery' ); ?></a>
                </div>
            <?php else : ?>
                <div class="mdp-messages mdc-text-field-helper-line mdc-text-field-helper-text mdc-text-field-helper-text--persistent">
                    <?php esc_html_e( 'Drag and drop or click on the form to add file', 'lottier-wpbakery' ); ?>
                </div>
            <?php endif; ?>
    ...
    public function render_file_dnd( $key, $tab, $field ) {
        ...
        <div class="mdp-dnd mdp-file">
            ...
            echo wp_sprintf(
                ...
                isset( $field[ 'label' ] ) ? esc_html( $field[ 'label' ] ) : esc_html__( 'File', 'lottier-wpbakery' )
            );
            ...
            if ( $file_exist ) :
                ...
                echo wp_sprintf(
                    ...
                    esc_html__( 'File exist', 'lottier-wpbakery' ),
                    esc_html__( 'Drop file here or click to upload', 'lottier-wpbakery' )
                );
            ...
            if ( $file_exist ) :
                ...
                echo wp_sprintf(
                    '%1$s | <a href="#" class="mdp-reset-key-btn">%2$s</a>',
                    esc_html__( 'Drag and drop or click on the form to replace file.', 'lottier-wpbakery' ),
                    esc_html__( 'Reset', 'lottier-wpbakery' )
                );
                ...
                echo wp_sprintf(
                    ...
                    esc_html__( 'Drag and drop or click on the form to add file.', 'lottier-wpbakery' ),
                    $field[ 'description' ] ?? '',
                    isset( $field[ 'file_types' ] )
                        ? esc_html__( 'Allowed file types:', 'lottier-wpbakery' ) . ' <b>' . esc_attr( $file_types ) . '</b>' : ''
                );
    ...
    public function render_sides( $key, $tab_slug, $field ) {
        ...
        echo wp_sprintf( '<div class="mdp-controls-sides-single mdp-controls-sides-lock%1$s" title="%2$s">
            ...
            esc_html__('Linked', 'lottier-wpbakery' ),
        ...
        foreach ( self::$css_sides as $number_key ) {
            ...
            UI::get_instance()->render_input(
                $values[ $number_key ],
	            $field[ 'labels' ][ $number_key ] ?? esc_html__( $number_key, 'lottier-wpbakery' ),
                '',
                [
        ...
        UI::get_instance()->render_select(
            self::$css_units,
            $unit,
            esc_html__( 'Units', 'lottier-wpbakery' ),
            '',
            [
    ...
    private function get_control_id( $key, $tab_slug, $field ) {
        ...
        $id = 'mdp_lottier_wpbakery_' . $tab_slug . '_settings_' . $key;
        return apply_filters( 'mdp_lottier_wpbakery_settings_control_id', $id, $key, $tab_slug, $field );
    ...
    private function get_control_name( $key, $tab_slug, $field ) {
        ...
        $name = 'mdp_lottier_wpbakery_' . $tab_slug . '_settings[' . $key . ']';
        return apply_filters( 'mdp_lottier_wpbakery_settings_control_name', $name, $key, $tab_slug, $field );
	...
	private function get_sides_values( $options, $slug, array $default = array() ): array {
		...
		return apply_filters( 'lottier_wpbakery_settings_side_values', $values, $slug );
    ...
    private function get_sides_unit( $options, $slug, array $default = array() ) {
        ...
        return apply_filters( 'lottier_wpbakery_settings_sides_unit', $unit, $slug );
    ...
    private function get_sides_linked( $options, $slug, array $default = array() ) {
        ...
        return apply_filters( 'lottier_wpbakery_settings_sides_linked', $linked, $slug );
	...
	public function render_unit_slider( $key, $tab_slug, $field ) {
		...
		UI::get_instance()->render_select(
			...
			esc_html__( 'Unit', 'lottier-wpbakery' ),
			'',
			[
				'id' => 'mdp_lottier_wpbakery_' . $tab_slug . '_settings_' . $key . '_unit',
				'name' => 'mdp_lottier_wpbakery_' . $tab_slug . '_settings[' . $key . '_unit' . ']',
			]
		);
		echo '</div>';
