...
final class Settings {
        ...
        <aside class="mdc-drawer">
            <div class="mdc-drawer__content">
                <nav class="mdc-list">
                    ...
                    <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Plugin settings', 'lottier-wpbakery' ) ?></h6>
					<?php
					...
					foreach ( $tabs as $tab => $value ) {
						...
						/** Prepare link. */
						$link = admin_url( 'admin.php?page=mdp_lottier_wpbakery_settings&tab=' . $tab );
	...
	private function support_link() {
	    ...
	    ?>
        <hr class="mdc-list-divider">
        <h6 class="mdc-list-group__subheader"><?php echo esc_html__( 'Helpful links', 'lottier-wpbakery' ) ?></h6>
        ...
        <a class="mdc-list-item" href="https://docs.merkulov.design/tag/lottier" target="_blank">
            ...
            <span class="mdc-list-item__text"><?php echo esc_html__( 'Documentation', 'lottier-wpbakery' ) ?></span>
        </a>

		<?php if ( TabActivation::get_instance()->is_activated() ) : /** Activated. */ ?>
            <a class="mdc-list-item" href="https://1.envato.market/lottier-wpbakery-support" target="_blank">
                <i class="material-icons mdc-list-item__graphic" aria-hidden="true">mail</i>
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Get help', 'lottier-wpbakery' ) ?></span>
            </a>
            <a class="mdc-list-item" href="https://1.envato.market/cc-downloads" target="_blank">
                ...
                <span class="mdc-list-item__text"><?php echo esc_html__( 'Rate this plugin', 'lottier-wpbakery' ) ?></span>
            </a>
		<?php endif; ?>
        ...
        <a class="mdc-list-item" href="https://1.envato.market/cc-merkulove" target="_blank">
            ...
            <span class="mdc-list-item__text"><?php echo esc_html__( 'More plugins', 'lottier-wpbakery' ) ?></span>
        </a>
		<?php
	...
	private function add_submenu_elementor( $options ) {
        ...
        add_submenu_page(
            $parent,
	        'Lottier ' . esc_html__( 'Settings', 'lottier-wpbakery' ),
	        'Lottier ' . esc_html__( 'for Wpbakery', 'lottier-wpbakery' ),
	        $options[ 'capability_settings' ] ?? 'manage_options',
            'mdp_lottier_wpbakery_settings',
            [ $this, 'options_page' ]
        );
	...
	private function add_menu_wordpress( $options ) {
        ...
        add_menu_page(
            'Lottier for WPBakery ' . esc_html__( 'Settings', 'lottier-wpbakery' ),
            'Lottier for WPBakery',
	        $options[ 'capability_settings' ] ?? 'manage_options',
            'mdp_lottier_wpbakery_settings',
            [ $this, 'options_page' ],
	...
	private function get_current_tab(): string {
		...
		return apply_filters( 'lottier_wpbakery_current_tab', $tab );
    ...
    private function render_nag_saved() {
        ...
        /** Render "Settings Saved" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Settings saved!', 'lottier-wpbakery' ) );
    ...
    private function render_nag_activation() {
        ...
        if ( TabActivation::get_instance()->is_activated() ) {
            ...
            UI::get_instance()->render_snackbar( esc_html__( 'Plugin activated successfully.', 'lottier-wpbakery' ), 'success', 5500 );
            return;

        }

        /** Render "Activation failed" message. */
        UI::get_instance()->render_snackbar( esc_html__( 'Invalid purchase code.', 'lottier-wpbakery' ), 'error', 5500 );
        ...
        <div class="mdc-drawer__header mdc-plugin-fixed">
            <a class="mdc-list-item mdp-plugin-title" href="#">
                <i class="mdc-list-item__graphic" aria-hidden="true">
                    <img src="<?php echo esc_attr( Plugin::get_url() . 'images/logo-color.svg' ); ?>" alt="<?php echo esc_attr( 'LottierWpbakery' ) ?>">
                </i>
                <span class="mdc-list-item__text">
                    <?php if ( 'wordpress' === Plugin::get_type() ) : ?>
                        <?php echo esc_html( 'Lottier for WPBakery' ); ?>
                    <?php else: ?>
                        <?php echo esc_html( 'Lottier' ); ?>
                    <?php endif; ?>
                </span>
                <span class="mdc-list-item__text">
                    <sup>
                        <?php esc_html_e( 'v.', 'lottier-wpbakery' ); ?>
            ...
            <button type="submit" name="submit" id="submit" class="mdc-button mdc-button--dense mdc-button--raised">
                <span class="mdc-button__label"><?php esc_html_e( 'Save changes', 'lottier-wpbakery' ) ?></span>
            </button>
        </div>
		<?php
	...
	public function get_options() {
        ...
        foreach ( $tabs as $tab_slug => $tab ) {
	        ...
	        $opt_name = "mdp_lottier_wpbakery_{$tab_slug}_settings";
