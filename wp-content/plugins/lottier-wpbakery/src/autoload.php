...
final class Autoload {
    ...
    public static function load( $class ) {
        ...
        /** Classes from Lottier for WPBakery. */
        $file_p = self::get_plugin_class_file( $class );

        /** If Class exists in Lottier for WPBakery - load it. */
    ...
    private static function get_unity_class_file( $class ) {
        ...
        return $file . self::$DS . str_replace( ['LottierWpbakery\\', '\\'], ['', self::$DS], $class ) . '.php';

    }

    /**
     * Build file path for classes from Lottier for WPBakery directory.
     *
     * @param string $class - Called class name.
     *
     * @static
