...
use Merkulove\LottierWpbakery\Unity\Cache;
use Merkulove\LottierWpbakery\Unity\Helper;
use Merkulove\LottierWpbakery\Unity\Plugin;
use Merkulove\LottierWpbakery\UninstallHelper;
use Merkulove\LottierWpbakery\Unity\EnvatoItem;
...
final class Uninstall {
    ...
    private function remove_settings() {
        ...
        foreach ( wp_load_alloptions() as $option => $value ) {
            if ( strpos( $option, 'mdp_lottier_wpbakery' ) === 0 ) {
                $settings[] = $option;
            }
        }
    ...
    public function get_uninstall_mode() {
        ...
        if ( isset( $uninstall_settings[ 'mdp_lottier_wpbakery_migration_settings' ] ) && $uninstall_settings[ 'mdp_lottier_wpbakery_migration_settings' ] ) { // Default value.
            $uninstall_settings = [
